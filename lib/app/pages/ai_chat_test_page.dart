...
import 'package:ex_bot/data/repositories/azure_coach_repository.dart';
...
class _AIChatTestPageState extends State<AIChatTestPage> {
  ...
  void initState() {
    ...
    _messages.add(
      'Configuration status: ${EnvironmentConfig.isConfigured ? "✅ Connected to Azure OpenAI" : "❌ Not configured"}',
    );
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final result = await _aiCoachRepository.sendMessage(message: userMessage, userId: 'test_user_001');
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                FloatingActionButton(onPressed: _isLoading ? null : _sendMessage, child: const Icon(Icons.send)),
              ],
            ),
          ),
        ],
