...
import 'package:ex_bot/data/ai_models/ai_message.dart';
import 'package:ex_bot/data/ai_models/ai_message_role.dart';
...
class AzureCoachRepository implements CoachRepository {
  ...
  Future<Either<Failure, AiMessage>> sendMessage({
    ...
    try {
      ...
      return Left(Failure.unknownFailure(message: 'Unexpected error: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, AiConversation>> getConversation({required String conversationId}) async {
  ...
  @override
  Future<Either<Failure, AiConversation>> createConversation({required String userId, String? initialContext}) async {
  ...
  Future<Either<Failure, AiConversation>> updateConversation({
    ...
    return Left(Failure.unknownFailure(message: 'Conversation update not yet implemented'));
  }
  ...
  Failure _handleDioError(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.receiveTimeout:
        return Failure.networkFailure(message: 'Request timeout. Please check your internet connection.');
      case DioExceptionType.badResponse:
        final statusCode = error.response?.statusCode;
        final message = error.response?.data?['error']?['message'] ?? 'Server error occurred';
